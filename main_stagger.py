...
def compose_experiments(dataset_name):
    ...
    # args_list.append((get_combining_experiment(suffix + "_WV3",
    #      [copy.deepcopy(rf), copy.deepcopy(lr), copy.deepcopy(nb), copy.deepcopy(xgb_batch), copy.deepcopy(dt)],
    #      [copy.deepcopy(arf), copy.deepcopy(srp_model), copy.deepcopy(hat), copy.deepcopy(nb_online)], window_size, n_curr_ref_retrain, threshold,
    #      threshold, n_first_fit, n_online, is_data_drift=is_data_drift, is_target_drift=is_target_drift, is_performance=is_performance_drift, comb_type='wv'), df.copy(deep=True)))
    
    return args_list

def main():
    dataset_name = 'stagger1K'
    args_list = []
