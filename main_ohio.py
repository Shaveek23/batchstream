...
def compose_experiments(dataset_name):
    ...
    n_curr_ref_retrain = 5000
    threshold = 0.02
    suffix = f"batch_{suffix_}_{n_curr_ref_retrain}_{threshold}"
    ...
    args_list.append((get_evidently_experiment(f"{suffix}_lgbm", rf, n_online=n_online, n_first_fit=n_first_fit, window_size=window_size,
       n_curr=n_curr_ref_retrain, data_stattest_threshold=threshold,
       data_drift=is_data_drift, target_drift=is_target_drift, is_performance=is_performance_drift), df.copy(deep=True)))
