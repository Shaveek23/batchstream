...
def compose_experiments(dataset_name):
    ...
    n_curr_ref_retrain = 150
    threshold_data = 0.01
    threshold_target = 0.05
    n_online = 20
    is_data_drift = True
    is_target_drift = True
    ...
    #    n_curr=n_curr_ref_retrain, data_stattest_threshold=threshold_data, target_stattest_threshold=threshold_target,
    #    data_drift=is_data_drift, target_drift=is_target_drift, is_performance=is_performance_drift), df.copy(deep=True)))
    ...
    args_list.append((get_combining_experiment(suffix + "_MV4", [copy.deepcopy(rf), copy.deepcopy(xgb_batch), copy.deepcopy(dt), copy.deepcopy(nb)],
        [copy.deepcopy(srp_model), copy.deepcopy(nb_online)], window_size, n_curr_ref_retrain,
        threshold_data, threshold_target, n_first_fit, n_online, is_data_drift=is_data_drift, is_target_drift=is_target_drift, is_performance=is_performance_drift, comb_type='mv'), df.copy(deep=True)))
    
    args_list.append((get_combining_experiment(suffix + "_DS4", [copy.deepcopy(rf), copy.deepcopy(xgb_batch), copy.deepcopy(dt), copy.deepcopy(nb)],
        [copy.deepcopy(srp_model), copy.deepcopy(nb_online)], window_size, n_curr_ref_retrain,
         threshold_data, threshold_target, n_first_fit, n_online, is_data_drift=is_data_drift, is_target_drift=is_target_drift, is_performance=is_performance_drift, comb_type='ds'), df.copy(deep=True)))
    
    return args_list

def main():
    dataset_name = 'comobility'
    args_list = []
