...
from experiments.second import get_combining_experiment, get_combining_exp_different_detectors
import copy
...
def compose_experiments(dataset_name):
   ...
   # xgb_batch = Pipeline([('xgb', xgb.XGBClassifier())])
    ...
    # args_list.append((get_combining_exp_different_detectors(suffix + "_DS7", [copy.deepcopy(dt)], [copy.deepcopy(lr_online), copy.deepcopy(nb_online)], window_size, n_curr_ref_retrain,
    #     n_first_fit, n_online, [0.01, 0.02], [0.01, 0.02], True, comb_type='ds'), df.copy(deep=True)))
    
    # args_list.append((get_combining_exp_different_detectors(suffix + "_MV7", [copy.deepcopy(dt)], [copy.deepcopy(lr_online), copy.deepcopy(nb_online)], window_size, n_curr_ref_retrain,
    #     n_first_fit, n_online, [0.01, 0.02], [0.01, 0.02], True, comb_type='mv'), df.copy(deep=True)))

    # args_list.append((get_combining_experiment(suffix + "_MV6", [copy.deepcopy(rf)], [copy.deepcopy(srp_model), copy.deepcopy(arf)], window_size, n_curr_ref_retrain, threshold,
    #     threshold, n_first_fit, n_online, is_data_drift=is_data_drift, is_target_drift=is_target_drift, is_performance=is_performance_drift, comb_type='mv'), df.copy(deep=True)))
    
    # args_list.append((get_combining_experiment(suffix + "_DS6", [copy.deepcopy(rf)], [copy.deepcopy(srp_model), copy.deepcopy(arf)], window_size, n_curr_ref_retrain, threshold,
    #     threshold, n_first_fit, n_online, is_data_drift=is_data_drift, is_target_drift=is_target_drift, is_performance=is_performance_drift, comb_type='ds'), df.copy(deep=True)))
